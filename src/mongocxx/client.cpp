...
client::client(const class uri& uri, const options::client& options) {
    ...
    if (options.command_listener_opts()) {
        mongoc_apm_callbacks_t* callbacks = mongoc_apm_callbacks_new();
        mongoc_apm_set_command_started_cb(callbacks, client::impl::command_started_wrapper);
        mongoc_client_set_apm_callbacks(
            new_client, callbacks, (void*)(*options.command_listener_opts()));
    }
