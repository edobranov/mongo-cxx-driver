...
TEST_CASE("Transaction tests", "[database]") {
    instance::current();
    ...
    auto successful_insert_test = [&](client_session session,
    ...
    SECTION("Transaction options are entirely implicit (default settings)") {
        successful_insert_test(mongodb_client.start_session(), {});
    }

    SECTION("Transaction options are set on session initialization") {
    ...
    SECTION("Transaction options are explicitly set") {
        options::transaction transaction_opts;
        transaction_opts.read_preference(read_preference());
    ...
    SECTION("Transaction commit can have a TransientTransactionError label") {
        auto db = mongodb_client["test"];
        auto coll = db["txn_test"];
        bool has_transient_error = false;
        ...
        mongodb_client["admin"].run_command(
            make_document(kvp("configureFailPoint", "failCommand"),
                          ...
                          kvp("data",
                              make_document(kvp("failCommands", make_array("insert", "update")),
                                            kvp("errorCode", 10107)))));
        session.start_transaction();
        ...
        try {
            ...
            // Contains "errorLabels" : [ "TransientTransactionError" ]
            INFO("Collection exception raw: " << bsoncxx::to_json(*(e.raw_server_error())) << "\n");

            // Prints "Failing command due to 'failCommand' failpoint: generic server error"
            INFO("Collection exception msg: " << e.what() << "\n");

            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // Intentionally do NOT abort to force TransientTransactionError from server on commit
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

            has_transient_error = e.has_error_label("TransientTransactionError");
        }

        REQUIRE(has_transient_error);
        ...
        try {
            ...
            // Contains "errorLabels" : [ "TransientTransactionError" ]
            INFO("Commit exception raw:     " << bsoncxx::to_json(*(e.raw_server_error())) << "\n");

            // Prints "Given transaction number 1 does not match any in-progress
            //         transactions.: generic server error"
            INFO("Commit exception msg:     " << e.what() << "\n");
