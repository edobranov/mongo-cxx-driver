...
// TODO: rename txn_opts_doc to more generalize opts_doc
bsoncxx::stdx::optional<read_concern> get_read_concern(document::view txn_opts_doc) {
    if (txn_opts_doc["readConcern"] && txn_opts_doc["readConcern"]["level"]) {
        read_concern rc;
        rc.acknowledge_string(string::to_string(txn_opts_doc["readConcern"]["level"].get_utf8().value));
        return rc;
    }
...
bsoncxx::stdx::optional<read_preference> get_read_preference(document::view txn_opts_doc) {
    if (txn_opts_doc["readPreference"] && txn_opts_doc["readPreference"]["mode"]) {
        read_preference rp;
        std::string mode = string::to_string(txn_opts_doc["readPreference"]["mode"].get_utf8().value);
        if (mode.compare("Primary") == 0) {
            rp.mode(read_preference::read_mode::k_primary);
        }
        else if (mode.compare("Primary Preferred") == 0) {  // TODO: check if this is the proper spelling
        ...
        else if (mode.compare("Secondary Preferred") == 0) {  // TODO: check if this is the proper spelling
            rp.mode(read_preference::read_mode::k_secondary_preferred);
        }
        else if (mode.compare("Nearest") == 0) {  // TODO: check if this is the proper spelling
...
document::value run_start_transaction_test(collection *coll, document::view operation, client_session* session) {
    (void) coll;
...
document::value run_commit_transaction_test(collection *coll, document::view operation, client_session* session) {
    (void) coll;
    (void) operation;
...
document::value run_insert_one_test(collection* coll, document::view operation, client_session* session) {
...
document::value run_insert_many_test(collection* coll, document::view operation, client_session* session) {
    using bsoncxx::builder::basic::kvp;
    using bsoncxx::builder::basic::make_document;
    document::view arguments = operation["arguments"].get_document().value;
...
std::map<std::string, std::function<document::value(collection*, document::view, client_session* session)>>
...
void run_transactions_tests_in_file(std::string test_path) {
    ...
    for (auto&& test : tests) {
        ...
        // TODO: Create the new client with Command Monitoring listeners enabled
        ...
        for (auto&& op : operations) {
            try {
                ...
                // TODO: support "collectionOptions" from YAML upon collection creation
                collection coll = db.create_collection(coll_name);
                ...
                if (op["arguments"] && op["arguments"]["session"]) {
                    ...
                    else if (op["arguments"]["session"].get_utf8().value.compare("session1") == 0) {
                ...
                if (op["result"] && op["result"]["errorContains"]) {
                    REQUIRE(0 == op["result"]["errorContains"].get_utf8().value.compare(std::string(e.what())));
                }
        ...
        client["admin"].run_command(make_document(kvp("configureFailPoint", "failCommand"),
                                                  kvp("mode", "off")));
